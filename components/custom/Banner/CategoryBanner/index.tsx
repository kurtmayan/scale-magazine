...
export default function CategoryBanner({
  ...
  const leftSectionBlog = blogs.slice(1, 3);
  const rightSectionFirstBlog = blogs[3];
  const rightSectionLastBlog = blogs[4];
  const remainingBlogs = blogs.slice(1, 6);

  console.log(
    `/blog/${category.toLowerCase()}/${primaryBlog.slug.current}}/${primaryBlog.slug.current}`,
  );
      ...
      <div className="xl:w-360 xl:mx-auto xl:py-20 p-5">
        ...
        <div className="hidden xl:grid grid-cols-2 gap-20">
          <div className="flex flex-col gap-22.5">
            <Link
              href={`/blog/${category.toLowerCase()}/${primaryBlog.slug.current}}/${primaryBlog.slug.current}`}
              className="hover:cursor-pointer relative aspect-video -z-10 w-179 h-214.75  "
            >
              <Image
            ...
            <div className="grid grid-cols-2 gap-22.5">
              {leftSectionBlog &&
                leftSectionBlog.map(({ title, slug, featuredImage }) => (
                  <Link
                    href={`/blog/${category.toLowerCase()}/${slug.current}`}
                    key={slug.current}
                  >
          ...
          <div className="flex flex-col gap-20">
            <Link
              href={`/blog/${category.toLowerCase()}/${primaryBlog.slug.current}`}
            ...
            {rightSectionFirstBlog && (
              <div className="flex flex-col gap-20">
                <Link
                  href={`/blog/${category.toLowerCase()}/${rightSectionFirstBlog.slug.current}`}
                >
                  <div className="flex flex-row gap-5 items-center">
                    <div className="aspect-square w-68.5 h-64.5 relative">
                      <Image
                        src={urlFor(rightSectionFirstBlog.featuredImage).url()}
                        alt={rightSectionFirstBlog.title}
                        fill
                        className="object-cover"
                      />
                    </div>
            ...
            {rightSectionLastBlog && (
              <Link
                href={`blog/${category.toLowerCase()}/${rightSectionLastBlog.slug.current}`}
              >
                <div className="flex flex-col gap-5">
                  <div className="w-170.75 h-155 aspect-square relative">
                    <Image
                      src={urlFor(rightSectionLastBlog.featuredImage).url()}
                      alt={rightSectionLastBlog.slug.current}
                      fill
                      className="object-cover"
                    />
                  </div>
        ...
        <div className="xl:hidden">
          <Link href={`/blog/${category}/${primaryBlog.slug.current}`}>
            ...
            {remainingBlogs.length >= 1 &&
              remainingBlogs.map(({ featuredImage, slug, title }) => (
                <Link
                  ...
                  key={slug.current}
                >
                  <div className="w-87.25 flex flex-cols gap-3">
